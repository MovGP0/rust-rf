...
#[test]
fn calculates_column_major_scalar_data_and_sample_covariance() {
    ...
    assert_eq!(scalar[(0, 0, 0)], 1.0);
    assert_eq!(scalar[(0, 1, 1)], 4.0);
    let covariance = set.covariance_s().unwrap();
    assert_eq!(covariance[(0, 0, 0)], 2.0);
    assert_eq!(covariance[(0, 0, 1)], 2.0);
    assert_eq!(covariance[(0, 1, 0)], 2.0);
    assert_eq!(covariance[(0, 1, 1)], 2.0);
}

#[test]
fn writes_metas_sdatcv_header_mean_and_covariance() {
...
#[test]
fn rejects_empty_and_single_observation_sets() {
...
fn one_port(frequency: Frequency, value: Complex64) -> Network {
