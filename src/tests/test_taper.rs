//! Linear, exponential, smooth-step, custom, and Klopfenstein taper tests.
...
/// Checks standard profile sample vectors and section lengths.
#[test]
fn generates_standard_taper_profiles() -> rust_rf::Result<()> {
    ...
    for (index, value) in values.iter().enumerate() {
        let normalized = index.to_f64().unwrap_or(f64::NAN) / 3.0;
        assert_relative_eq!(
            *value,
    ...
    assert_relative_eq!(values[2], f64::midpoint(11.7, 4.6), epsilon = 1.0e-12);
    assert_relative_eq!(values[4], 4.6, epsilon = 1.0e-12);
    Ok(())
}

/// Checks custom normalized and Klopfenstein profiles plus reflection validation.
#[test]
fn generates_custom_and_klopfenstein_profiles() -> rust_rf::Result<()> {
    ...
    assert_relative_ne!(first_values[4], second_values[4], epsilon = 1.0e-12);
    Ok(())
}

/// Checks medium/section construction and the fully cascaded taper network.
#[test]
fn builds_sections_and_cascaded_taper_network() -> rust_rf::Result<()> {
...
fn taper(
    ...
    stop: f64,
    sections: usize,
) -> rust_rf::Result<Taper1D<DefinedGammaZ0>> {
...
fn test_frequency() -> rust_rf::Result<Frequency> {
