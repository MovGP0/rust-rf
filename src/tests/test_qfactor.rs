...
#[test]
fn fits_six_parameter_resonator_response() {
...
#[test]
fn calculates_circle_unloaded_q_weights_and_bandwidth() {
    let network = synthetic_resonator(
        0.9e9,
        1.1e9,
        11,
...
#[test]
fn fits_seven_parameter_phase_delay_model() {
...
#[test]
fn fits_eight_parameter_frequency_dependent_leakage_model() {
...
#[test]
fn matches_npl_mat58_reference_datasets() {
    ...
    let figure27 = fixture_network("Figure27.txt");
    let absorption = QFactor::new(figure27, ResonanceType::Absorption, None, None)
    ...
    let result7 = reflection
        .fit_method(QFitMethod::Nlqfit7)
        .expect("Table 6c27 NLQFIT7 should fit");
    assert_relative_eq!(
        result7.resonant_frequency_hz,
        3.652_938e9,
        max_relative = 2.0e-6
    );
    assert_relative_eq!(result7.loaded_q, 708.0, max_relative = 2.0e-2);

    let mut figure23 = fixture_network("Figure23.txt");
    for point in 0..figure23.frequency_points() {
        let frequency = figure23.frequency.values_hz()[point];
        figure23.s[(point, 0, 0)] *= Complex64::from_polar(
            1.0,
            -std::f64::consts::TAU * frequency * 1.2 / SPEED_OF_LIGHT,
        );
    }
    let peak = (0..figure23.frequency_points())
        .max_by(|left, right| {
            figure23.s[(*left, 0, 0)]
                .norm_sqr()
                .total_cmp(&figure23.s[(*right, 0, 0)].norm_sqr())
        })
        .expect("Figure 23 should contain samples");
    let seed_frequency = figure23.frequency.values_hz()[peak];
    let seed_q = 5.0 * seed_frequency
        / (figure23.frequency.values_hz()[figure23.frequency_points() - 1]
            - figure23.frequency.values_hz()[0]);
...
fn fixture_network(name: &str) -> Network {
    ...
    for line in fs::read_to_string(path)
        .expect("Q-factor fixture should be readable")
        .lines()
    {
        let line = line.trim();
        ...
        let values = line
            ...
            .map(|value| {
                value
                    .parse::<f64>()
                    .expect("fixture values should be numeric")
            })
            .collect::<Vec<_>>();
        assert!(
            values.len() >= 3,
            "fixture rows need frequency, real, imaginary"
        );
    ...
    let frequency =
        Frequency::from_values(ndarray::Array1::from_vec(frequency_ghz), FrequencyUnit::GHz)
            .expect("fixture frequency should be valid");
    let points = frequency.points();
    Network::new(
        frequency,
        Array3::from_shape_vec((points, 1, 1), scattering)
            .expect("fixture scattering shape should be valid"),
        Array2::from_elem((points, 1), Complex64::new(50.0, 0.0)),
    )
    .expect("fixture network should be valid")
}
use std::fs;
