...
fn connects_a_two_port_between_external_ports() {
    ...
    let port_two = Circuit::port(frequency.clone(), "Port 2", Complex64::new(50.0, 0.0))
        .expect("port should be valid");
    let through = Arc::new(through_network(frequency));
    ...
    let incident = circuit
        .incident_waves(&array![1.0, 0.0], &array![0.0, 0.0])
        .expect("incident waves should be constructed");
    ...
    let outgoing = circuit
        .outgoing_waves(&incident)
        .expect("outgoing waves should solve");
    assert_complex_close(outgoing[(0, 0)], Complex64::new(0.0, 0.0));
    assert_complex_close(outgoing[(0, 3)], Complex64::new(2.0_f64.sqrt(), 0.0));

    let (voltages, currents) = circuit
        .external_voltages_currents(&array![1.0, 0.0], &array![0.0, 0.0])
        .expect("external voltage and current should solve");
    ...
    let (internal_voltages, internal_currents) = circuit
        .internal_voltages_currents(&array![1.0, 0.0], &array![0.0, 0.0])
        .expect("internal voltage and current should solve");
    ...
    let active_vswr = circuit
        .active_vswr(&excitation)
        .expect("active VSWR should solve");
    ...
    let reduced = circuit.reduced().expect("circuit should reduce");
    assert_eq!(reduced.network_count(), 3);
    let reduced_network = reduced.network().expect("reduced circuit should solve");
    ...
    let updated = circuit
        .updated_networks(&HashMap::from([("through".to_owned(), replacement)]))
        .expect("circuit networks should update");
    let updated_network = updated.network().expect("updated circuit should solve");
...
fn rejects_duplicate_ports_and_missing_external_ports() {
    let frequency = Frequency::from_hz(array![1.0e9]).expect("frequency should be valid");
    let network = Arc::new(through_network(frequency));
    assert!(
        Circuit::new(vec![vec![
...
fn through_network(frequency: Frequency) -> Network {
    let points = frequency.points();
    ...
    let mut network = Network::new(frequency, s, z0).expect("network should be valid");
