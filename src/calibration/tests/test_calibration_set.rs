...
#[test]
fn creates_dot_calibrations_and_corrected_standard_sets() {
    let frequency =
    ...
    let measured_sets = standards
        ...
        .map(|standard| {
            ...
            .expect("measured set should be valid")
        })
        .collect::<Vec<_>>();
    ...
    .expect("dot calibration set should be constructed");
    ...
    let corrected = calibration_set
        .apply_cal(&raw)
        .expect("raw network should be corrected by every calibration");
    ...
    let corrected_sets = calibration_set
        .corrected_sets()
        .expect("corrected standards should be grouped");
    ...
    .expect("cartesian calibrations should be constructed");
    assert_eq!(combinations.len(), 8);
}

#[test]
fn rejects_misaligned_dot_sets() {
    ...
    let measured_sets = vec![
        NetworkSet::new(vec![ideals[0].clone()], None).expect("set should be valid"),
        NetworkSet::new(vec![ideals[1].clone(), ideals[1].clone()], None)
            .expect("set should be valid"),
        NetworkSet::new(vec![ideals[2].clone()], None).expect("set should be valid"),
    ];
